...
double Moon::phase(const QDate &date)
{
    ...
    long    jd = date.toJulianDay();
    // The lunar phase
    double  ph = 0.0;
    // The portion of a lunar phase which elapse per day
    double phase_per_day = 1/(MEAN_LUNAR_CYCLE/8);
    ...
    // Now get the first date of the new moon of year month entered
...
double Moon::flmoon(int n,int nph,long& jd,double& frac)
{
    ...
    if(nph == 0 || nph == 2) // New or Full - sun earth moon inline
        xtra += ( ( 0.1734-3.93e-4 * t ) * std::sin(RAD*sun_anomaly) ) - ( 0.4068 * std::sin(RAD*moon_anomaly) );
    else // 1st quarder last quarter - moon 90 deegree left or right of the earth and suns line 
        xtra += ( ( 0.1721-4.0e-4 * t ) * std::sin(RAD*sun_anomaly) ) - ( 0.6280 * std::sin(RAD*moon_anomaly) );

    // This parts easy just put the integer and fractional parts right
    int_part = (int)( xtra >= 0.0 ? std::floor(xtra) : ceil(xtra-1.0) );
    jd += int_part;
    frac = xtra - int_part;
